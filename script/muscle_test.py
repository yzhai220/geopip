...
# Remove '-' to get unaligned data.
for k, v in multiAlign.iteritems():
    multiAlign[k] = v.replace('-', '')

# Output unaligned data.
...
###############################################################################
# Use MUSCLE to estimate an alignment.
os.chdir(dataLoc)
os.system(dir_main + '/software/muscle -in msa.fasta -out msa_aligned.fas')
os.chdir(resultFolder)

# Read estimated alignment (in phylip format).
msaTrueFile = dataLoc + '/msa_aligned.fas'
multiAlign = read_fasta(msaTrueFile)
